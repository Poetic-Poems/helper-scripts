...
| `list-escalations.sh [OWNER-REPO-FILTER] [JQ-FILTER]` | Lists every open issue labelled `enabler-escalation` or `pw::pager` in each repository cloned under `~/Code/Poetic-Poems` and `~/Code/Pullwright`, as one JSON array, optionally filtered by an owner/repo grep -E filter (e.g., `'^Pullwright'`) and a `jq` expression (e.g., `'.number > 1000'`). |
